...
//EsferaconProfe clase 4 de noviembre del 2024

float	posferacox = 0.0f,
		posferacoy = 0.0f,
		veloinicial = 20.0f,
		anguloparabol = 90.0f,
		myVariable = 0.0f,
...
//extra de 4 de diciembre del 2024
...
typedef struct _frame
{
	...
	float movAvionX, movAvionY, movAvionZ, rotAvionZ;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].movAvionX = movAvionX;
	KeyFrame[FrameIndex].movAvionY = movAvionY;
	KeyFrame[FrameIndex].movAvionZ = movAvionZ;
	KeyFrame[FrameIndex].rotAvionZ = rotAvionZ;
...
void resetElements(void)
{
	...
	movAvionX = KeyFrame[0].movAvionX;
	movAvionY = KeyFrame[0].movAvionY;
	movAvionZ = KeyFrame[0].movAvionZ;
	rotAvionZ = KeyFrame[0].rotAvionZ;
...
void interpolation(void)
{
	...
	incMovAvionX = (KeyFrame[playIndex + 1].movAvionX - KeyFrame[playIndex].movAvionX) / i_max_steps;
	incMovAvionY = (KeyFrame[playIndex + 1].movAvionY - KeyFrame[playIndex].movAvionY) / i_max_steps;
	incMovAvionZ = (KeyFrame[playIndex + 1].movAvionZ - KeyFrame[playIndex].movAvionZ) / i_max_steps;
	incRotAvionZ = (KeyFrame[playIndex + 1].rotAvionZ - KeyFrame[playIndex].rotAvionZ) / i_max_steps;
}
...
{
	aspas += 4.0f;
	//movAvionX = 20.0f * cos (myVariable);
	//movAvionZ = 20.0f * sin (myVariable);

	//Calculos variables para esfera con profe clase 4 de noviembre del 2024

	if (empiezatiropara)
	{
		if (posferacoy <= 0.0f && posferacoxante != 0.0f)
		{
			//empiezatiropara = false;
			myVariable = 0.0f;
			veloinicial = 2 * veloinicial / 3;
			anguloparabol = 0.8 * anguloparabol;
			posferacoy = 2.0f;
			posferacoxante = posferacox;
		}
		else
		{
			posferacox = posferacoxante + veloinicial * myVariable * cos(glm::radians(anguloparabol));
			posferacoy = veloinicial * myVariable * sin(glm::radians(anguloparabol)) - 0.5f * 9.78 * pow(myVariable, 2);
			myVariable += 0.02f;
		}
	}
	...
	if (caidalibre)
	{
		if (poswy < 29.0f)
		{
			poswy = poswante -0.5 * 9.78 * pow(extraVariable, 2);
			poswante = poswy;
			extraVariable += 0.002f;
			if (poswante <= 0.0f)
			{
				//poswy = 0.0f;
				caidalibre = false;
		...
		{
				poswante = poswy + (-0.5 * 9.78 * pow(extraVariable, 2));
				extraVariable += 0.002f;
				poswy = poswante;
		}
	}
	...
	if (rebote)
	{
		if (poswante <= 0.0f && !caidalibre)
		{
			extraVariable = 0.4f;
			poswy = poswante + 2.5 * extraVariable - (0.5 * 9.78 * pow(extraVariable, 2));
			poswx = poswx + 0.1 * cos(glm::radians(20.0f)) * extraVariable;
			poswante = poswy;
			extraVariable += 0.002f;
		}
		if (poswante <= 0.12f && poswx > 0.0f)
			{
				rebote = false;
				poswy = 30.0f;
				poswx = 0.0f;
	...
	if (play)
	{
		...
		else
		{
			...
			movAvionX += incMovAvionX;
			movAvionY += incMovAvionY;
			movAvionZ += incMovAvionZ;
			rotAvionZ += incRotAvionZ;
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Pratica X 2025-1", NULL, NULL);
	if (window == NULL) {
	...
	Model esferaconprofe("resources/objects/Esfera/Esfera_LSVC.obj");
	Model dobleu("resources/objects/w/dobleu.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/***   Figura con primitivas  **/
		// ----------------- Avioneta ----------------------------//
		glBindVertexArray(VAO[1]);
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(movAvionX, 10.0 + movAvionY, movAvionZ - 12));
		tmp = modelOp = glm::rotate(modelOp, glm::radians(rotAvionZ), glm::vec3(0.0f, 0.0f, 1.0f));
		modelOp = glm::scale(modelOp, glm::vec3(5.0f, 1.0f, 1.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 0.0f, 1.0f, 0.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //A

		modelOp = glm::translate(tmp, glm::vec3(0.0f, 0.0f, -1.5f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f, 1.0f, 2.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 0.0f, 0.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		...
		modelOp = glm::translate(tmp, glm::vec3(-3.0f, 0.5f, 0.0f));
		modelOp = glm::scale(modelOp, glm::vec3(1.0f, 2.0f, 1.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 0.0f, 0.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //D

		modelOp = glm::translate(tmp, glm::vec3(2.75f, 0.0f, 0.0f));
		tmp = modelOp = glm::rotate(modelOp, glm::radians(aspas), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		modelOp = glm::scale(tmp, glm::vec3(0.48f, 0.5f, 2.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 1.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //F

		//Biblioteca a partir del cubo unitario
		modelOp = glm::mat4(1.0f);
		tmp = modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(85.0f, 6.5f, -10.0f));
		...
		modelOp = glm::translate(tmp, glm::vec3(1.0f, -2.5f, 17.5f));
		modelOp = glm::scale(modelOp, glm::vec3(2.0f, 10.0f, 15.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 0.0f, 0.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //D

		modelOp = glm::translate(tmp, glm::vec3(12.5f, -2.5f, 24.0f));
		modelOp = glm::scale(modelOp, glm::vec3(21.0f, 10.0f, 2.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 1.0f, 0.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //E

		modelOp = glm::translate(tmp, glm::vec3(24.0f, -2.5f, 17.5f));
		modelOp = glm::scale(modelOp, glm::vec3(2.0f, 10.0f, 15.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 0.0f, 0.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		...
		tmp = modelOp = glm::translate(tmp, glm::vec3(0.0f, 22.5f, -0.5f));
		modelOp = glm::scale(modelOp, glm::vec3(40.0f, 30.0f, 15.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 0.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //B

		modelOp = glm::translate(tmp, glm::vec3(2.0f, 20.0f, 0.5f));
		modelOp = glm::scale(modelOp, glm::vec3(20.0f, 10.0f, 10.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 0.0f, 1.0f, 1.0f);
		glBindTexture(GL_TEXTURE_2D, t_white);
		glDrawArrays(GL_TRIANGLES, 0, 36); //C
		...
		//Esfera con profe en clase 4 de noviembre del 2024

		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f + posferacox, 2.0f + posferacoy, 20.0f));
		modelOp = glm::scale(modelOp, glm::vec3(2.0f));
		staticShader.setMat4("model", modelOp);
		staticShader.setVec3("dirLight.specular", glm::vec3(1.0f, 1.0f, 1.0f));
		esferaconprofe.Draw(staticShader);

		//extra en clase 4 de diciembre del 2024

		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(-15.0f + poswx, poswy, -35.0f));
		staticShader.setMat4("model", modelOp);
		staticShader.setVec3("dirLight.specular", glm::vec3(1.0f, 1.0f, 1.0f));
		dobleu.Draw(staticShader);
...
void my_input(GLFWwindow* window, int key, int scancode, int action, int mode) 
{
	...
	//Esfera co profe clase 4 de noviembre del 2024

	if (key == GLFW_KEY_F10 && action == GLFW_PRESS)
		empiezatiropara = !empiezatiropara;
